...
public class Cannon {
	...
	private Graphics2D drawCannon(Graphics2D g) {
		...
		g.fillOval((x - 25)/2, GamePanel.HEIGHT - 100, 100, 100);
	...
	private Graphics2D drawBalls(Graphics2D g) {
		...
		for (int i = 0; i < balls.size(); i++) {
			...
			balls.get(i).update();
			if(GamePanel.click == true && GamePanel.cursorX > balls.get(i).getX() && GamePanel.cursorY > balls.get(i).getY() && GamePanel.cursorY < balls.get(i).getY() + balls.get(i).getDiameter() && GamePanel.cursorX < balls.get(i).getX() + balls.get(i).getDiameter()) {
			balls.removeAll(balls);
		}
	...
	private Graphics2D drawButtons(Graphics2D g) {
		...
		f = new Font("Calibri", Font.BOLD, 32);
		g.setFont(f);
		g.drawString("START", startButtonX + 7, startButtonY + startButtonHeight - 10);
